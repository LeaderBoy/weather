...
- (id)initWithFrame:(CGRect)frame itemOffset:(CGFloat)itemOffset imageArray:(NSArray *)images titleArray:(NSArray *)titles menuDelegate:(id<TYCircleMenuDelegate>)menudelegate {
    ...
    if ((self = [super initWithFrame:frame])) {
        ...
        _centerButton = [[UIButton alloc]initWithFrame:CGRectMake(20, self.bounds.size.height-70, 50, 50)];
        UIImage *image = [UIImage imageNamed:@"100"];
        
        [_centerButton setImage:image forState:UIControlStateNormal];
        [_centerButton addTarget:self action:@selector(onCenterButtonClick:) forControlEvents:UIControlEventTouchUpInside];
